...
export const getUsers = async (
    ...
    pageSize: number = 10
        ...
        try {
            const response = await BaseService.post({url: API_PATHS.GET_USERS ,payload:  {
                searchCondition: {
                  keyword: keyword || "",
                  role: role || "",
...
export const changePassword = async (values: ValuesChangePassword) => {
  const response = await BaseService.put({url: API_PATHS.CHANGE_PASSWORD, payload:{
        userId: user.id,
      ...
      export const changeStatusUser = async (
        ...
        await BaseService.put({url: API_PATHS.CHANGE_STATUS_USER, payload: {
            userId: userId,
            status: checked,
          }});
      ...
      export const changeUserRole = async (userId: string, role: UserRole) => {
        await BaseService.put({url: API_PATHS.CHANGE_ROLE, payload: {
            userId: userId,
            role,
          }});
